...
class Main extends PluginBase implements Listener{
				...
				default:
					{
						if($sender instanceof Player){ // player only commands
							switch($command->getName()){
								case "commandplayer":
									{
										if($sender->hasPermission("DSR.command.DSR")){
											return true;
											getPlayer->teleport($x,$y,$z,$level);
											getPlayer->addPlayer;
										}
										else{
											$sender->sendMessage($this->getTranslation("no-permission"));
	...
	public function onTeleport(EntityTeleportEvent $event){
		...
		public function (PlayerMoveEvent $event){
        $player = $event->getPlayer();
        $level = $player->getLevel();
        $arena = DontStopRunning
